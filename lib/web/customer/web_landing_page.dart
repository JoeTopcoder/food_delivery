...
class _HeroSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      final vPad = isMobile ? 40.0 : 64.0;
      return Container(
        width: double.infinity,
        constraints: BoxConstraints(minHeight: isMobile ? 380 : 500),
        ...
        child: Stack(
          children: [
            if (!isMobile) ...[
              ...
              Positioned(right: 180, bottom: -80, child: _Bubble(180, const Color(0xFFFF6B35), 0.07)),
            ...
            Padding(
              ...
              child: isMobile
                  ? Column(
                      ...
                      children: [
                        _HeroLeft(onBrowseFood: onBrowseFood, onSignUp: onSignUp, isMobile: true),
                      ],
                    )
                  : Row(
                      crossAxisAlignment: CrossAxisAlignment.center,
                      children: [
                        Expanded(flex: 55, child: _HeroLeft(onBrowseFood: onBrowseFood, onSignUp: onSignUp, isMobile: false)),
...
class _HeroLeft extends StatelessWidget {
  ...
  final VoidCallback onSignUp;
  final bool isMobile;
  const _HeroLeft({required this.onBrowseFood, required this.onSignUp, required this.isMobile});
      ...
      children: [
        Container(
          ...
          child: const Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            SizedBox(width: 5),
            Text('Fast Delivery · Cayman Islands',
        ...
        Text(
          isMobile
              ? 'Hundreds of restaurants, fast doorstep delivery.'
              : 'Browse hundreds of restaurants, track your order live,\nand enjoy fast doorstep delivery.',
          style: const TextStyle(color: Color(0xFF94A3B8), fontSize: 15, height: 1.6),
        ),
        ...
        Wrap(spacing: 16, runSpacing: 8, children: const [
          _TrustBadge(Icons.star_rounded, '4.8★ Rating'),
          _TrustBadge(Icons.storefront_rounded, '200+ Restaurants'),
          _TrustBadge(Icons.timer_rounded, '30 min avg'),
        ]),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      if (isMobile) {
        ...
        cardGrid = Column(children: [
          Row(children: [Expanded(child: cards[0]), const SizedBox(width: 20), Expanded(child: cards[1])]),
          const SizedBox(height: 20),
          Row(children: [Expanded(child: cards[2]), const SizedBox(width: 20), Expanded(child: cards[3])]),
        ]);
      } else {
        cardGrid = Row(children: [
      ...
      return Container(
        ...
        padding: EdgeInsets.fromLTRB(hPad, isMobile ? 48 : 72, hPad, isMobile ? 48 : 72),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Container(
                ...
                child: Text(widget.badge,
                    style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: widget.gradient.colors.first)),
              ),
            ]),
            ...
            Row(children: [
              Text(widget.ctaLabel,
                  style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: widget.gradient.colors.first)),
...
class _FeaturedSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return Container(
        color: Colors.white,
        padding: EdgeInsets.fromLTRB(hPad, 52, hPad, 52),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(
            '🔥 Popular Right Now',
            style: TextStyle(
          ...
          const Text('Top-rated restaurants ordered by customers like you',
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (r.deliveryFee != null) ...[
                  ...
                  Text('${AppConstants.currencySymbol}${r.deliveryFee!.toStringAsFixed(2)} delivery',
...
class _HowItWorksSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      final steps = [
        _Step('01', Icons.search_rounded, 'Browse & Choose',
            'Search restaurants, explore menus, and add items to your cart with a tap.'),
        _Step('02', Icons.credit_card_rounded, 'Place Your Order',
            'Checkout securely with your wallet, card, or cash on delivery.'),
        _Step('03', Icons.delivery_dining_rounded, 'Track & Enjoy',
            'Follow your driver in real-time on the map and enjoy your meal.'),
      ];

      return Container(
        color: const Color(0xFFF8FAFC),
        padding: EdgeInsets.fromLTRB(hPad, isMobile ? 48 : 64, hPad, isMobile ? 48 : 64),
        child: Column(children: [
          Text(
            'How It Works',
            style: TextStyle(fontSize: isMobile ? 24 : 30, fontWeight: FontWeight.w800, color: const Color(0xFF1E293B), letterSpacing: -0.5),
          ),
          const SizedBox(height: 8),
          const Text('Get your favourite food in three easy steps',
              textAlign: TextAlign.center, style: TextStyle(fontSize: 15, color: Color(0xFF64748B))),
          SizedBox(height: isMobile ? 32 : 52),
          if (isMobile)
            Column(
              children: steps.asMap().entries.map((e) => Padding(
                padding: EdgeInsets.only(bottom: e.key < 2 ? 32 : 0),
                child: e.value,
              )).toList(),
            )
          else
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Container(height: 2, color: const Color(0xFFE2E8F0), margin: const EdgeInsets.symmetric(horizontal: 4)),
        ]),
      ),
    );
  }
...
class _StatsBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return Container(
        padding: EdgeInsets.symmetric(horizontal: isMobile ? 24 : 80, vertical: isMobile ? 36 : 52),
        decoration: const BoxDecoration(
        ...
        child: isMobile
            ? Wrap(
                ...
                runSpacing: 24,
                children: [
                  SizedBox(width: constraints.maxWidth / 2, child: const _Stat('200+', 'Restaurants')),
                  SizedBox(width: constraints.maxWidth / 2, child: const _Stat('1,000+', 'Drivers')),
                  SizedBox(width: constraints.maxWidth / 2, child: const _Stat('50K+', 'Orders')),
                  SizedBox(width: constraints.maxWidth / 2, child: const _Stat('4.8 ★', 'Avg Rating')),
                ],
              )
            : Row(
                mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                  const _Stat('200+', 'Restaurants'),
                  _StatDivider(),
                  const _Stat('1,000+', 'Active Drivers'),
                  _StatDivider(),
                  const _Stat('50K+', 'Orders Delivered'),
                  _StatDivider(),
                  const _Stat('4.8 ★', 'Average Rating'),
                ],
              ),
      );
    });
...
class _Stat extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(children: [
    Text(value, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.w900)),
    const SizedBox(height: 4),
    Text(label, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white70, fontSize: 13)),
  ]);
}
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          if (isMobile) ...[
            ...
            Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Expanded(child: _FooterCol('Customers', ['Browse', 'My Orders', 'Loyalty', 'Track'])),
                Expanded(child: _FooterCol('Partners', ['Restaurant', 'Driver Sign Up', 'Benefits', 'Sales'])),
                Expanded(child: _FooterCol('Company', ['About', 'Privacy', 'Terms', 'Support'])),
              ],
            ),
          ] else ...[
            Row(
