...
/// Restaurant Success Agent — every metric here is computed deterministically
/// (cancellation rate, prep time, rating, refund rate); the AI only narrates
/// which partners need attention and why. Read-only, no approval needed.
class AdminRestaurantSuccessScreen extends StatefulWidget {
  const AdminRestaurantSuccessScreen({super.key});

  @override
  State<AdminRestaurantSuccessScreen> createState() => _AdminRestaurantSuccessScreenState();
}

class _AdminRestaurantSuccessScreenState extends State<AdminRestaurantSuccessScreen> {
  bool _loading = false;
  String? _error;
  ...
  Future<void> _generate() async {
    ...
    try {
      final res = await Supabase.instance.client.functions.invoke('restaurant-success-agent');
      final data = res.data;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Restaurant Success', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w600)), centerTitle: true),
      body: ListView(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text('\$${(m['revenue'] as num).toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                    ],
                  ),
                ),
              );
