    ...
    try {
      ...
      if (mounted) {
        if (activated) {
          AppSnackbar.success(
            context,
            'HotBite+ ${planType == 'pro' ? 'Pro' : 'Basic'} is now active!',
          );
        } else {
          AppSnackbar.warning(
            context,
  ...
  Future<void> _cancelSub(String subscriptionId) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Cancel HotBite+?'),
        content: const Text(
    ...
    try {
      ...
      if (mounted) {
        if (activated) {
          AppSnackbar.success(
            context,
            'Switched to HotBite ${newPlan == 'pro' ? 'Pro' : 'Basic'}!',
          );
        } else {
          AppSnackbar.warning(
            context,
                    ...
                    child: OutlinedButton.icon(
                      ...
                      label: Text(
                        _subscribingPlan == 'change'
                            ? 'Switching...'
                            : 'Switch to HotBite ${activeSub.planType == 'basic' ? 'Pro' : 'Basic'}',
                      ),
