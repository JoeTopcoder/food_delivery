-- Migration 121: Admin dashboard summary RPC
-- Single SECURITY DEFINER function that returns all dashboard stats.
-- Runs as the function owner (bypasses RLS) so admins always get real counts.
-- Each table access is wrapped independently so a missing table or error
-- returns 0 instead of killing the whole function.
