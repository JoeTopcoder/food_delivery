-- Allow cancellation of orders in 'draft' status.
--
-- 'draft' orders have not had payment captured yet, so there is never
-- a penalty or refund to process — we simply mark them cancelled and return.
...
BEGIN
  ...
  -- Draft orders: cancel immediately, no penalty, no refund (nothing was charged)
  ...
  IF v_penalty > 0 AND v_order.driver_id IS NOT NULL THEN
    v_driver_user_id := public.resolve_driver_user_id(v_order.driver_id);

    IF v_driver_user_id IS NOT NULL THEN
      INSERT INTO wallets (user_id, balance)
      VALUES (v_driver_user_id, v_penalty)
