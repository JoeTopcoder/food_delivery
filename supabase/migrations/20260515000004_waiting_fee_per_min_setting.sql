-- Add waiting_fee_per_min to ride_pricing_settings so admins can configure
-- the per-minute rate charged when a driver waits or pauses a ride.

ALTER TABLE ride_pricing_settings
  ADD COLUMN IF NOT EXISTS waiting_fee_per_min NUMERIC DEFAULT 75.0;

-- Update update_ride_status to read the rate from settings instead of
-- hardcoding 75.0 in the start_waiting branch.
...
BEGIN
  ...
  -- start_waiting branch: read rate from pricing settings
  IF p_start_waiting THEN
    ...
    END IF;
    SELECT COALESCE(waiting_fee_per_min, 75.0) INTO v_rate
      FROM ride_pricing_settings WHERE active = true LIMIT 1;
    v_rate := COALESCE(v_rate, 75.0);
    UPDATE ride_requests
       SET waiting_started_at  = NOW(),
           waiting_fee_per_min = v_rate
     WHERE id = p_ride_id;
    RETURN jsonb_build_object('message', 'Waiting fee started', 'rate', v_rate);
  END IF;
  ...
  UPDATE ride_requests SET
    ...
    paused_at           = CASE
                            WHEN p_new_status = 'ride_paused'  THEN NOW()
