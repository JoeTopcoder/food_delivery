-- ═══════════════════════════════════════════════════════════════════
-- Migration 043: Refund wallet balance when a wallet-paid order is cancelled
-- ═══════════════════════════════════════════════════════════════════
-- When an order paid via wallet is cancelled, the order total is
-- refunded back to the user's wallet MINUS the cancellation fee.
-- Also fixes the column reference (total_amount, not total).
...
BEGIN
  ...
  IF v_order.payment_method = 'wallet' THEN
    ...
    IF v_refund > 0 THEN
      -- Credit the net refund (total minus fee) back to wallet
      UPDATE wallets SET
    ...
    -- If there's a penalty but refund covered it, no separate penalty deduction needed
    -- If penalty > total (shouldn't happen), record the penalty transaction
  ...
  ELSE
    -- Non-wallet orders: just deduct penalty from wallet if applicable
    IF v_penalty > 0 THEN
      INSERT INTO wallet_transactions (user_id, amount, type, payment_method, status, order_id, description)
      VALUES (p_user_id, -v_penalty, 'penalty', 'system', 'completed', p_order_id,
        'Cancellation fee for order #' || UPPER(LEFT(p_order_id::text, 8)));
