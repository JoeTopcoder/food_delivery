-- Add paused_at to ride_requests so the driver's pause timer can be restored
-- exactly when they return to the active ride screen.
-- update_ride_status sets it to NOW() on ride_paused and clears it on ride_started.

ALTER TABLE ride_requests ADD COLUMN IF NOT EXISTS paused_at TIMESTAMPTZ;
...
BEGIN
  ...
  UPDATE ride_requests SET
    ...
    -- Record exactly when the driver paused; clear it when they resume.
    paused_at           = CASE
                            WHEN p_new_status = 'ride_paused'  THEN NOW()
                            WHEN p_new_status = 'ride_started' THEN NULL
                            ELSE paused_at
                          END,
    -- Clear waiting fee the instant the driver resumes.
