-- Phase 9 (server-side enforcement): a STUDENT lunch order requires active
-- student benefits. Verbatim re-creation of place_lunch_order with ONE added
-- guard right after the existing student-authorization check. Ordering for
-- yourself is untouched — normal ordering is never blocked.
...
BEGIN
  ...
  -- Student benefits must be active for a STUDENT order (self orders unaffected).
  IF v_is_student AND NOT public.student_benefits_active(p_student_id) THEN
    RAISE EXCEPTION 'Student benefits are currently unavailable. Please update the student ID to restore access.'
