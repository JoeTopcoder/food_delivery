-- Fold a driver's cash float into their payout: final = earnings owed - float.
--   cash_float > 0  -> driver holds platform cash (COD) -> reduces their payout
--   cash_float < 0  -> platform owes driver (fronted restaurant cash) -> adds
-- Only drivers whose NET is positive are billed; when billed, both the earnings
-- balance and the float are settled to zero. Restaurants have no float.
...
AS $$
  ...
  FROM drivers d
  WHERE (d.total_earnings - coalesce(d.total_paid_out,0)
...
BEGIN
  ...
  -- Drivers: net = earnings owed - float. Only net-positive with bank info.
  ...
  -- Settle earnings AND float to zero for the billed drivers.
  UPDATE drivers d
