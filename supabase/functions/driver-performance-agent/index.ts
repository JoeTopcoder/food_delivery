// driver-performance-agent — Driver Performance Agent (HotBite AI Operations)
      ...
      const res = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: `You are the HotBite Driver Performance Agent. You're given 30-day metrics for every approved driver, ranked by a deterministic at_risk_score (higher = more concerning). compliance_flag comes from a different HotBite agent (Driver Compliance) — "expired" means their license has already lapsed and is the single most urgent thing to mention regardless of their delivery performance; "expiring_soon" is worth a mention too. Write a short briefing (100-150 words): name the 1-3 drivers most worth checking in on and the specific reason (cancellation rate, slow delivery time, or compliance flag — cite the actual numbers), and note any standout performer. Drivers with order_count under 3 and no compliance flag have insufficient data — don't flag them. You may ONLY ever recommend a check-in, coaching conversation, or compliance follow-up — never recommend deactivation or suspension, that decision requires a formal human review. Never invent a number not present in the data. Plain text, no markdown.`,
            },
