// ops-report-agent — consolidated read-only report agents (HotBite AI Operations)
//
...
const SALES_FROM_EMAIL = Deno.env.get('SALES_FROM_EMAIL') ?? 'HotBite Partnerships <onboarding@resend.dev>'
  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: `You are the HotBite Customer Retention drafting assistant. Draft a short, warm win-back email to a returning customer who hasn't ordered in a while. Rules:
...
Respond ONLY with JSON: { "subject": string, "body": string, "discount_percent": number }`,
  ...
  const subject = String(parsed.subject ?? `We miss you at HotBite`).slice(0, 200)
...
async function retentionApproveOutreach(customerId: string, decision: string, finalSubject: string, finalBody: string, finalDiscountPercent: number | undefined, admin: { id: string }): Promise<Response> {
  ...
  let subject = (finalSubject ?? '').trim() || 'We miss you at HotBite'
  ...
  if (OPENAI_API_KEY && ranked.length > 0) {
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: `You are the HotBite Dispatch Optimization Agent. You're given available drivers ranked by distance (already sorted, nearest first), EXCEPT drivers with compliance_flag "expired" are always sorted last regardless of distance — that's a deterministic safety rule, not your judgment, and it comes from a different HotBite agent (Driver Compliance). Explain in 2-3 sentences why the top driver is the recommended pick. If the nearest driver has an expired license and got pushed down the list, say so explicitly so the admin understands why they weren't recommended first. An admin still confirms the assignment manually. Never invent a driver or distance not in the data. Plain text.` },
  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: `You are the HotBite Marketing Content Agent. Given a brief, draft the requested marketing content (social post, promo copy, recruitment ad, etc.) for HotBite — a food/grocery/ride delivery marketplace. Rules:
...
// the lead via Resend. Grounded in real HotBite stats — never invents
  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: `You are the HotBite Restaurant Sales drafting assistant. Draft a short, warm outreach email inviting this restaurant to join HotBite as a delivery partner. Rules:
...
- Sign off as "The HotBite Partnerships Team".
Respond ONLY with JSON: { "subject": string, "body": string }`,
  ...
  const completion = await res.json()
  const parsed = JSON.parse(completion.choices?.[0]?.message?.content ?? '{}')
  const subject = String(parsed.subject ?? `Partner with HotBite`).slice(0, 200)
  const body = String(parsed.body ?? '').slice(0, 4000)
...
async function salesApproveOutreach(leadId: string, decision: string, finalSubject: string, finalBody: string, admin: { id: string }): Promise<Response> {
  ...
  const subject = (finalSubject ?? '').trim() || 'Partner with HotBite'
...
async function answerDatabaseQuestion(question: string, history: { role: string; content: string }[], admin: { id: string }): Promise<Response> {
  ...
  const messages: Record<string, unknown>[] = [
    {
      role: 'system',
      content: `You are the HotBite "Ask AI" assistant, built for admins to ask anything about the real business data in plain conversation. You can explore and query the actual production database (strictly read-only — every query is enforced read-only at the database level, so you cannot write no matter what).
