// automation-workflow-runner — Workflow Station (HotBite AI Operations)
//
...
const MARKETING_FROM_EMAIL = Deno.env.get('SALES_FROM_EMAIL') ?? 'HotBite Marketing <onboarding@resend.dev>'
...
async function weeklySocialCampaign(workflowId: string): Promise<WorkflowResult> {
  ...
  const promptContext = usingFallback
    ? 'No menu items on file yet — write a general brand-awareness post inviting people to try HotBite for the first time.'
    : hasOrderSignal
  ...
  const captionRes = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: `You write social media captions for HotBite, a food delivery app. Given real menu/order data (never invent a dish, restaurant, or order count not given to you), write exactly 3 distinct ready-to-post captions (Instagram/Facebook style, with relevant emoji, under 200 characters each). If no order-count data is provided, describe the items as menu highlights, not "best sellers" or "most popular". Respond ONLY with JSON: { "captions": [string, string, string] }`,
        },
...
async function createBirthdayPromoCode(
  ...
  const { data: inserted, error: insertErr } = await serviceClient
    ...
    .insert({
      code,
      description: 'Happy Birthday reward — HotBite',
      discount_type: 'fixed',
