...
function buildSystemPrompt(role: string, orderCtx: string): string {
  const base = `You are the HotBite AI assistant for a food delivery platform in the Cayman Islands. Be warm, concise, and helpful. Keep responses SHORT — 1 to 2 sentences max — this is a live voice call.${orderCtx}`
  switch (role) {
