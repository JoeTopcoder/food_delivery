...
const INTENT_PATTERNS: Array<{ intent: Intent; pattern: RegExp }> = [
  ...
  { intent: 'faq',                     pattern: /\b(FAQ|frequently asked|common (question|issue|problem)|how does (it|the app|ordering|payment|delivery) work|how do i (use|start|begin|place|pay|track)|what is (the app|HotBite|this service))\b/i },
...
function buildSystemPrompt(
  ...
  const intentGuidance: Record<string, string> = {
    ...
    subscription_billing:    'The user has a question about subscription billing. Subscriptions are billed on a recurring cycle (monthly or annually) to the saved payment method. The charge appears as "HotBite Premium" on statements. Check BILLING HISTORY for recent subscription charges.',
  ...
  const baseRules = `You are Aria, HotBite's senior customer experience specialist. You are professional, warm, articulate, and highly knowledgeable — responding like a well-educated human expert who genuinely cares about each customer and knows their account inside out.
${langInstruction}
...
- HotBite Premium costs $9.99/month or $89.99/year (2 months free).
...
- If you receive a suspicious call claiming to be HotBite, hang up and report via Help > Report Fraud.
...
- Social media support: also available via @HotBiteSupport on major platforms.
...
${contextBlock}`
  ...
  if (role === 'driver') {
    return `You are Aria, the HotBite driver support assistant. You help delivery drivers complete their deliveries efficiently and answer any questions about the platform, earnings, and policies.
${langInstruction}
