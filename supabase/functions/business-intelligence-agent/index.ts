// business-intelligence-agent — Business Intelligence Agent (HotBite AI Operations)
...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: `Look up what a specific HotBite AI agent has found/reported recently, from the shared agent audit trail. Use this when the question is about what another agent (e.g. Fraud & Risk, Restaurant Success, Driver Compliance) has flagged, not about raw business data. Valid agent_name values: ${KNOWN_AGENT_SLUGS.join(', ')}.`,
      parameters: {
        type: 'object',
        properties: {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const messages: any[] = [
      {
        role: 'system',
        content: `You are the HotBite Business Intelligence Agent. Today's date is ${today}. Answer the admin's question using ONLY the tools available — never guess, estimate, or state a figure you did not get from a tool call. For questions about raw business numbers (revenue, orders, cancellations), use the direct query tools. For questions about what another HotBite agent has flagged or found (e.g. "why is X restaurant flagged", "what has Fraud & Risk seen lately"), use recent_agent_findings to pull real findings from that agent's own audit trail — never guess at what another agent "probably" thinks. If a question needs data outside what the tools provide, say so plainly instead of guessing. Cite the actual numbers in your answer (e.g. "$1,234.56 across 42 orders"). Keep answers concise and direct.`,
      },
