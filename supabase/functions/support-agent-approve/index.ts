// support-agent-approve — AI Support Agent (HotBite AI Operations, v1)
...
const FROM_EMAIL = Deno.env.get('SUPPORT_FROM_EMAIL') ?? 'HotBite Support <onboarding@resend.dev>'
  ...
  try {
    ...
    if (RESEND_API_KEY) {
      const html = `<!DOCTYPE html><html><body style="font-family:-apple-system,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#1a1a2e;">
        ...
        ${creditIssued ? `<p style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:12px 16px;color:#16a34a;">We've added a $${creditAmount.toFixed(2)} credit to your HotBite wallet.</p>` : ''}
      ...
      const emailResp = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: FROM_EMAIL,
          to: [ticket.email],
          subject: `Re: Your HotBite support request (${ticket.category})`,
          html,
        }),
      })
