...
// (HotBite AI Operations, consolidated)
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (role === 'customer') {
      systemPrompt = `You are the HotBite customer support drafting assistant. You write a draft reply and a recommended action for a human admin to review — you never contact the customer directly and you never move money yourself.
...
- Keep the reply concise, warm, and specific. Sign off as "The HotBite Support Team".
...
Respond ONLY with JSON: { "draft_reply": string, "suggested_action": string, "suggested_amount": number|null, "confidence": number, "reasoning": string }`
      ...
      systemPrompt = `You are the HotBite Restaurant Support drafting assistant. You write a draft reply for a human admin to review — you never contact the restaurant directly.
...
- confidence is 0.0-1.0. Sign off as "The HotBite Partner Support Team".
...
Respond ONLY with JSON: { "draft_reply": string, "suggested_action": string, "confidence": number, "reasoning": string }`
      ...
      systemPrompt = `You are the HotBite Driver Support drafting assistant. You write a draft reply for a human admin to review — you never contact the driver directly.
...
- suggested_action must be exactly one of: "none", "reply_only", "escalate".
- confidence is 0.0-1.0. Sign off as "The HotBite Driver Support Team".
