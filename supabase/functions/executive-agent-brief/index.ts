// executive-agent-brief — Executive Intelligence Agent (HotBite AI Operations)
      ...
      const openaiRes = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: `You are the HotBite Executive Intelligence Agent. You write a short, direct executive briefing (120-180 words) narrating ONLY the numbers given to you below — never invent, estimate, or extrapolate a figure that isn't present. The support_agent_signal block comes from the Customer Support Agent's own activity log (a different AI agent) — treat escalations and credits issued as real operational signal, not just a metric to restate. Call out the single most important risk or win, and if support_agent_signal shows a notable pattern (e.g. several escalations), name it explicitly. Plain text, no markdown headers.`,
            },
